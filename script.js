document.addEventListener("DOMContentLoaded", function () {
  ...
  const responses = {
    ...
    // 元々の trivia 配列（実際の雑学などの候補）
    trivia: [
      "豆知識: 蜜蜂は花の位置をダンスで仲間に伝えます🐝！",
      "雑学: キリンの首は7つの頸椎でできているんですよ🦒。",
      "面白い話: 人間は1日約2000回、まばたきをすると言われています😉。",
      "便利な話: バナナの皮は靴磨きに使えるってご存知ですか？🍌"
      // ※ このリストにもっとエントリーを追加することも可能です
    ]
  };
  ...
  // -----------------------------
  const scaryTrivia = [
    "怖い話 1: 夜中に部屋の隅で、いないはずの足音が聞こえてくる…😨",
    "怖い話 2: 鏡に映る自分の影が、動いているように感じる奇妙な現象…👻",
    "怖い話 3: 廃墟と化した家から、誰もいないはずのささやき声が聞こえる…😱",
    "怖い話 4: 古い写真の中に、一人だけ全く違う場所に写っている人物が…😰",
    "怖い話 5: 真夜中、ふと目覚めたら、窓の外に誰かの視線を感じた…😖"
    // ※ さらにエントリーを追加すれば、バリエーションも増やせます
  ];
  ...
  async function generateBotReply(userText) {
    ...
    // もし「怖い話」が含まれていれば、他の返答を無視して怖い話のみを返す
    if (lowerText.includes("怖い話")) {
    ...
    // 「豆知識」「雑学」「面白い話」「便利な話」など
    if (
      lowerText.includes("豆知識") || lowerText.includes("雑学") ||
      lowerText.includes("面白い話") || lowerText.includes("便利な話")
    ) {
